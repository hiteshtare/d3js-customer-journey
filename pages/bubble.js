...
d3.json("bubble.json", function (error, root) {
  ...
  if (selectedSegment && !customerName) {

    zoom(selectedSegment);
  } else if (selectedSegment && customerName) {
    const customers = selectedSegment.children;
    const selectedCustomer = customers.find((element) => {
      return element.data.id === customerName;
    });
    zoom(selectedCustomer);
  }
});